...
###########################################################################################
    ...
    def step12(self):
        ...
        def calczi(i):
            return (self.table6[i-1][1]-self.xbbar)/self.S if i < len(self.table6) else math.ceil((self.table6[-1][1]-self.xbbar)/self.S)
        ...
        def calclaplac(i):
            ...
            if i <= 0:
                ...
                sign = "-" if f"{zi}".count('-') > 0 else ""
                _zi = f"{zi}".replace('-', '')
                lap = 0
                try:
                    lap = self.laplac[float(f"{float(_zi):.1f}")][int(_zi[4])]
                except:
                    lap = self.laplac[float(f"{float(_zi):.1f}")][0]
                return float(f"{sign}0.{lap}")
        _ni_ = 0
        _nmi_ = 0
        for index in range(len(xi)-1):
            ...
            _col4 = calclaplac(index+1)
            col5 = col4+0.5
            col6 = _col4+0.5
            col7 = float(col6)-float(col5)
            col8 = col7*self.N
            col9 = 0
            _nmi_ += col8
            _ni_ += col3
            ...
            # \t{col5}\t{col6}\t{col7}\t{col8}")
            print(
                f"{col4+0.5:.4f}\t{_col4+0.5:.4f}")  # \t{col8:.4f}\t{col9:.4f}")
